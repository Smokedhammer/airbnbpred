...
st.set_page_config(page_title="Time Complexity Calculator", page_icon="🤖")
...
def complexity_calculate(text_input):
  response = openai.ChatCompletion.create(
  model="gpt-3.5-turbo",
  messages=[
      {"role": "user", "content": text_input+" What is the time complexity for this code "}
...
st.title("Welcome to the time complexity calculator tool powered by the latest AI technology.")

st.subheader("Enter the code and get it's time complexity along with an explanation.")

    
complexity=st.text_area("CTRL + Enter to simplify. (Click anywhere on the app after typing for mobile users)")

st.write(complexity_calculate(complexity))
